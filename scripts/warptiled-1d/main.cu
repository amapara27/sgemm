...
__global__ void sgemm_1d_wt(const float *a, const float *b, float *c, int K, int M, int N, float alpha, float beta) {
    // block tile dims
    int bk = 8; // step size along K
    int bm = 64; // rows of C
    int bn = 64; // cols of C

    int tm = 8; // 8 elements per thread
    ...
    totalPerBT = bm * bn; // each block tile is bm x bn
    threadsPerBT = totalPerBT / tm; // total / amt per thread

    // ensure dimensions align: total results per blocktile / results per thread = threads per blocktile
    assert(threadsPerBT == blockDims.x); 
    ...
    int iColA = threadIdx.x % bk;
    int iRowA = threadIdx.x / bk;
    int iColB = threadIdx.x % bn;
    int iRowB = threadIdx.x / bn;
